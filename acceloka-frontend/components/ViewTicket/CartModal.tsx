    ...
    <>
      ...
      >
        <div className="space-y-4">
          {cart.length === 0 ? (
            <p className="text-center text-dark-3 py-10">Your cart is empty.</p>
          ) : (
            <ul className="space-y-4">
              {cart.map((item) => (
                <li
                  key={item.ticketCode}
                  className="flex justify-center font-mono overflow-visible px-3"
                >
                  {/* Ticket UI */}
                  <div
                    className="relative flex flex-col sm:flex-row items-stretch my-2 rounded-xl border-2 border-yellow-500 shadow-sm w-full"
                    style={{ background: "#f5c842", overflow: "visible" }}
                  >
                    <div
                      className="absolute top-1/2 sm:top-1/2 left-0 sm:left-0 -translate-y-1/2 -translate-x-1/2 w-7 h-7 rounded-full border-2 border-yellow-500 z-10"
                      style={{ background: "#f0ede6" }}
                    />

                    <div
                      className="absolute top-1/2 sm:top-1/2 right-0 sm:right-0 -translate-y-1/2 translate-x-1/2 w-7 h-7 rounded-full border-2 border-yellow-500 z-10"
                      style={{ background: "#f0ede6" }}
                    />

                    <div
                      className="absolute top-0 bottom-0 border-l-2 border-dashed border-yellow-500 z-10 hidden sm:block"
                      style={{ right: 160 }}
                    />

                    <div
                      className="absolute left-0 right-0 border-t-2 border-dashed border-yellow-500 z-10 block sm:hidden"
                      style={{ bottom: 104 }}
                    />

                    {/* ticket info */}
                    <div className="flex-1 flex flex-col gap-1.5 p-4 sm:pr-16 pb-8 sm:pb-4">
                      <span className="text-base sm:text-lg font-bold tracking-wider text-dark-1 leading-tight">
                        {item.ticketName}
                      </span>
                    ...
                    <div className="flex flex-row sm:flex-col justify-between sm:justify-center items-center sm:items-end gap-2 px-6 sm:px-4 py-6 sm:py-3 bg-dark-1/5 sm:bg-transparent w-full sm:w-40 rounded-b-xl sm:rounded-none">
                      <div className="flex items-center gap-2">
                        ...
                        >
                          <MinusCircleIcon size={22} weight="fill" />
                        </button>
                      ...
                      <div className="flex flex-col items-end">
                        <p className="font-bold text-dark-1 text-base sm:text-lg">
                          Rp.{" "}
                          {(item.quantity * item.price).toLocaleString("id-ID")}
                        </p>
                        <div
                          onClick={() => removeFromCart(item.ticketCode)}
                          className="text-error flex items-center gap-1 cursor-pointer text-xs mt-1"
                        >
